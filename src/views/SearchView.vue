    ...
    <v-container>
      ...
      <v-row>
        <v-col
          ...
          sm="6"
          md="4"
          v-for="movie in this.$store.state.searchResults"
          :key="movie.imdbID"
        >
        ...
        <v-btn
          v-if="
            this.$store.state.searchResults.length <
              this.$store.state.foundResults &&
            this.$store.state.foundResults > 0
          "
          :loading="this.$store.state.status === 'PENDING_MORE'"
          :disabled="this.$store.state.status === 'PENDING_MORE'"
          @click="this.$store.dispatch('showMoreFetchMovies', true)"
          >Show more</v-btn>
      </v-row>
    </v-container>
  </div>
