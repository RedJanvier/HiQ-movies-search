...
const store = createStore({
  ...
  mutations: {
    storeMovies (state, data, isNextPage) {
      const movies = data.Search.map(movie => ({...movie, bookmarked: false}));
      if (!isNextPage) {
    ...
    removeBookmark(state, movieID) {
      state.searchResults = state.searchResults.map(movie => movie.imdbID === movieID ? ({...movie, bookmarked: false}) : movie);
      state.bookmarks = state.bookmarks.filter(movie => movie.imdbID !== movieID);
    },
    addBookmark(state, movieID) {
      state.searchResults = state.searchResults.map(movie => movie.imdbID === movieID ? ({...movie, bookmarked: true}) : movie);
      state.bookmarks.push(state.searchResults.filter(movie => movie.imdbID === movieID)[0]);
    },
    incrementPage(state) {
      state.page++;
    },
  ...
  actions: {
    ...
    async fetchMovies({commit, state}, isNextPage) {
      ...
      const res = await fetch(`${state.apiUrl}?s=${state.query}${state.year ? '&y=' + state.year : ''}${state.page ? '&page=' + state.page : ''}&apiKey=${state.apiKey}`);
    ...
    async fetchMovieSingle({commit, state}, movieID) {
      commit('updateStatus', 'PENDING');
      const res = await fetch(`${state.apiUrl}?i=${movieID}&apiKey=${state.apiKey}`);
      const data = await res.json();
